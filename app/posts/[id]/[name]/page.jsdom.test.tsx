...
const mockGetContactModalProps = vi.fn()
...
vi.mock('components/PostFavoriteButton', () => ({
  default: () => <button>favorite</button>,
}))

vi.mock('components/DeletePostButton', () => ({
...
describe('<Page />', () => {
  ...
  describe('if the authenticated user is the seller', () => {
    beforeEach(() => {
      mockAuth.mockResolvedValue({ id: '0' } as Session)
    })
  ...
  describe("if the authenticated user isn't the seller", () => {
    beforeEach(() => {
      mockAuth.mockResolvedValue({ id: '1' } as Session)
