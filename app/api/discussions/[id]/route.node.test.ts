...
vi.mock('libs/pusher/server', () => ({
  default: { trigger: vi.fn((...args: any[]) => mockPusherTrigger(...args)) },
}))
...
describe('GET', () => {
  test('422 - invalid id', async () => {
    const response = await GET(new NextRequest('http://-'), {
  ...
  test('422 - invalid csrf token', async () => {
    ...
    const response = await GET(new NextRequest('http://-?csrfToken=token'), {
      params: Promise.resolve({ id: new mongoose.Types.ObjectId().toString() }),
    })
    const data = await response.json()
  ...
  test('403 - forbidden', async () => {
    ...
    const response = await GET(new NextRequest('http://-'), {
      params: Promise.resolve({ id: discussion._id.toString() }),
    })
    const data = await response.json()
  ...
  test('200 - get the discussion', async () => {
    ...
    mockAuth.mockResolvedValue({ id: buyer._id.toString() } as Session)
    mockVerifyCsrfTokens.mockReturnValue(true)
    mockDbConnect.mockResolvedValue({} as Connection)
...
describe('PUT', () => {
  ...
  test("409 - can't send a message if the buyer is null", async () => {
    ...
    mockAuth.mockResolvedValue({ id: seller._id.toString() } as Session)
    mockDbConnect.mockResolvedValue({} as Connection)
  ...
  describe('update the unseen messages', () => {
    test('204 - last messages seen', async () => {
      ...
      const response = await PUT(
        new NextRequest('http://-', { method: 'PUT' }),
        { params: Promise.resolve({ id: discussion._id.toString() }) },
      )
